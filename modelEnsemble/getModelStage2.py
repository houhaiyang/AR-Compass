...
MAX_NB_CHARS = 24
amino_acids = list("ARNDCEQGHILKMFPSTWYVXUBZ")
class_num = 15
...
ar_name_to_label = {'aminoglycoside': 1,
                    'bacitracin': 2,
                    'beta_lactam': 3,
                    'chloramphenicol': 4,
                    'fosfomycin': 5,
...
clsid_to_class_name = {1: 'aminoglycoside',
                       2: 'bacitracin',
                       3: 'beta_lactam',
                       4: 'chloramphenicol',
                       5: 'fosfomycin',
...
class embedding_CNN_attention(nn.Module):
    def __init__(self, vocab_size, embedding_dim, class_size, dropout_value=0.5,
                 MAX_SEQUENCE_LENGTH=600, kernel_size=20, pool_kernel_size=0, stride=1,
...
class FocalLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        target_one_hot = torch.zeros_like(log_prob)
        target_one_hot.scatter_(1, target.unsqueeze(1), 1)
        if self.weight is not None:
            weight = self.weight.unsqueeze(0)
            weight = weight.repeat(target_one_hot.size(0), 1)
            target_one_hot = target_one_hot * weight
        loss = -self.alpha * (1 - prob) ** self.gamma * log_prob * target_one_hot
        loss = loss.sum(dim=1)
...
def create_separate_sequence(fa_file, amino_acids=amino_acids): # 24种氨基酸
    texts = []
...
def create_set(fastaPath, max_seq_len=600):
    ...
    query_x = pad_sequences(query_sequences, maxlen=max_seq_len, padding='post')  # 序列填充，后面补0，长度600

    query_ids = create_ids_for_sequences(fastaPath)

    return query_x,query_ids

def ARGDatasetCNN(fastaPath, max_seq_len=600, batch_size=128):
...
def ARGDatasetLSTM(fastaPath, max_seq_len=600, batch_size=128):
    query_x_test, query_ids = create_set(fastaPath, max_seq_len=max_seq_len)
...
def getModelStage2(model_type='CNN', group='100_600'):
    if model_type == 'CNN':
        ...
        model = model.cuda()
        model.load_state_dict(torch.load(f'models/Stage2/{model_name}'))
        return model
...
if __name__ == "__main__":
    import os
    os.chdir('D:/BGI/05.ARG/AR-Compass')

    # model = getModelStage2(model_type='CNN',length_type='all',group='100_600')
